                        ...
                        <div class="row">
                            <div class="mb-3 col-md-6">
                                ...
                                <label class="custom-switch">
                                    <input type="checkbox" name="status" value="1" {{ old('status', $frame->status) == 1 ? 'checked' : '' }}>
                            ...
                            <div class="mb-3 col-md-6">
                                ...
                                <label class="custom-switch">
                                    <input type="checkbox" name="free_paid" value="1"  {{ old('free_paid', $frame->free_paid) == 1 ? 'checked' : '' }}>
                                    <span class="switch-slider"></span>
                                </label>
                            </div>
                        </div>
...
// Load existing frame data on page load
document.addEventListener('DOMContentLoaded', function() {
    const frameData = @json($frame->data ?? '');
    const logoData = @json($frame->logosection ?? '');
    const imageUrl = "{{ asset('storage/' . $frame->image) }}";
    ...
    // Load existing data
    if (frameData) {
        frameCodeField.value = typeof frameData === 'string' ? frameData : JSON.stringify(frameData, null, 2);
    }
    if (logoData) {
        logoCodeField.value = typeof logoData === 'string' ? logoData : JSON.stringify(logoData, null, 2);
    }
